...
def create_core():
    ...
    if response.status_code == 200:
        ...
        if SOLR_CORE in cores:
            ...
            subprocess.run([solr_bin, "create", "-c", SOLR_CORE, "-solr-url", f"http://localhost:{SOLR_PORT}"], stdout=subprocess.DEVNULL)
...
def delete_core():
    ...
    if response.status_code == 200:
        ...
        if SOLR_CORE in cores:
            ...
            subprocess.run([solr_bin, "delete", "-c", SOLR_CORE, "-solr-url", f"http://localhost:{SOLR_PORT}"], stdout=subprocess.DEVNULL)
